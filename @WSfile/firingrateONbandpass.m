function firingrateONbandpass(obj, varargin)
    ...
    % Add the folder where your h5 files are to MATLAB's path
...
%     optargs = {100 2000 20 1 29 12 -200 200 0.01 'D:\Temp\From MATLAB 2020'};   % firing < 15 Hz
    ...
    % firing at higher frequencies - this was added to clean artifacts in
    % VClamp.
    ...
    % plotting zoomed in figure  
        ...
        figure('name', strcat(obj.file, ' (', num2str(sweepNumber), ') - zoom')); % naming figure file
        plot(x,yFiltered,'k','LineWidth',1)
        xmin=timeToStartCountingAPs+1;
        xmax=xmin+3;
        ...
        % adding scale bar
        xscale = 1;
        line([xmin,xmin+xscale],[ymin,ymin],'Color','k')
        line([xmin,xmin],[ymin,ymin+((ymax-ymin)/10)],'Color','k')
        text(xmin+(xmax-xmin)/130,ymin+((ymax-ymin)/30),strcat(num2str(xscale)," s"))
