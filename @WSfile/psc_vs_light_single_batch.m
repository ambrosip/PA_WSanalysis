...
%     end
    ...
    for pulseOnset = lightPulseStart.'
        ...
        chargeInSweep = trapz(dataSubsetForCharge,1); % pA x data point
        chargeInSweep = chargeInSweep * 1/samplingFrequency; % pA x second

        % put all charges from a particular sweep in a row (each column is
        % a pulse)
        chargesInSweep = [chargesInSweep, chargeInSweep];
    ...
    chargesAllSweeps = [chargesAllSweeps; chargesInSweep];
...
dataCell = [mouseNumber, ...
    ...
    mean(chargesAllSweeps), ...
    std(chargesAllSweeps), ...
...
oPSCvariableNamesCharge = cell(1,length(chargesInSweep));
...
oPSCvariableNamesChargeAVG = cell(1,length(chargesInSweep));
oPSCvariableNamesChargeSTD = cell(1,length(chargesInSweep));
...
for lightPulse = 1:length(lightEvokedCurrentsAmp)
    oPSCvariableNamesChargeAVG(lightPulse) = {strcat(num2str(lightPulse), 'oPSC(pA*s)AVG')};
    oPSCvariableNamesChargeSTD(lightPulse) = {strcat(num2str(lightPulse), 'oPSC(pA*s)STD')};
...
labeledData = cell2table(dataInCellFormat, 'VariableNames', ...      
    ...
    oPSCvariableNamesChargeAVG{:}, ...
    oPSCvariableNamesChargeSTD{:}, ...
